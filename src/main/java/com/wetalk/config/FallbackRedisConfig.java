...
/**
 * Dev profile 内存版 Redis，替代 Mockito mock。
 * 使用 ConcurrentHashMap 存储 key-value，支持 TTL 过期。
 */
@Configuration
@Profile("dev")
public class FallbackRedisConfig {
    ...
    // ═══════════════════════════════════════════════
    //  InMemoryValueOperations — ValueOperations 接口内存实现
    // ═══════════════════════════════════════════════
    ...
    private static class InMemoryValueOperations implements ValueOperations<String, String> {
        ...
        @Override public Boolean multiSetIfAbsent(Map<? extends String, ? extends String> map) {
            ...
            for (String key : map.keySet()) {
                if (store.containsKey(key)) { allAbsent = false; break; }
            }
        ...
        @Override public List<String> multiGet(Collection<String> keys) {
            evict();
        ...
        @Override public Long increment(String key, long delta) {
            ...
            store.compute(key, (k, v) -> {
                long n = (v == null ? 0 : Long.parseLong(v)) + delta;
                holder.set(n);
        ...
        @Override public Double increment(String key, double delta) {
            AtomicLong holder = new AtomicLong();
            store.compute(key, (k, v) -> {
                double n = (v == null ? 0 : Double.parseDouble(v)) + delta;
                holder.set((long) n);
                return String.valueOf(n);
            });
        ...
        @Override public Integer append(String key, String value) {
            ...
            store.compute(key, (k, v) -> {
                String result = (v == null ? "" : v) + value;
                holder[0] = result.length();
                return result;
            });
            return holder[0];
        }
    ...
    // ═══════════════════════════════════════════════
    //  InMemorySetOperations — SetOperations 接口内存实现
    // ═══════════════════════════════════════════════
